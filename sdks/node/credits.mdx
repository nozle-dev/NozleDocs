...
Transfers are subject to the Engine's exact transfer policy and source eligibility rules. Retry an uncertain transfer with the same key and payload.
...
`nozle.checkAndDeduct({ customerId, feature, credits })` remains available for the legacy Lago-wallet path. New product-credit integrations should use `usage.check()` and `usage.track()`.
