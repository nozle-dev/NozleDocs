...
| `baseUrl` | `string` | `http://localhost:8080` | API origin used for billing, entitlements, credits, entities, usage, and margin. |
| `eventsUrl` | `string` | `http://localhost:3000` | Event API origin used by `track()` and credit-system discovery. |
...
flowchart LR
  ...
  SDK -->|baseUrl| API[Billing, entitlements, credits, usage, margin]
