...
```bash
npm install @nozle-js/node@0.7.0
```
...
| `baseUrl` | `string` | `https://api.nozle.app/engine` | API origin used for entitlements, credits, entities, usage, and margin. |
| `eventsUrl` | `string` | `https://api.nozle.app/core` | Core API origin used for customer writes, billing resources, `track()`, credit-system discovery, and Entity subscriptions. Use `https://api.nozle.app/core` in production. |
...
export const nozle = new Nozle({
  ...
  eventsUrl: process.env.NOZLE_EVENTS_URL ?? 'https://api.nozle.app/core',
})
```
