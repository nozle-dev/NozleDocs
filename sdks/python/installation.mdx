...
| `base_url` | `str` | `https://api.nozle.app/engine` | Entitlement, credit, Entity, usage, and margin API origin. |
| `events_url` | `str` | `https://api.nozle.app/core` | Core API origin for customer writes, billing resources, metered events, credit-system discovery, and Entity subscriptions. Use `https://api.nozle.app/core` in production. |
| `timeout` | `float` | `10` | Per-request timeout in seconds. |

Version `0.7.0` defaults to these public service bases. Set both URLs explicitly for local or self-hosted deployments. Trailing slashes are removed automatically.

```mermaid
flowchart LR
  App[Python backend] --> SDK[nozle-sdk]
  SDK -->|base_url| API[Entitlements, credits, usage, margin]
  SDK -->|events_url| Core[Customers, billing resources, metered events]
```
