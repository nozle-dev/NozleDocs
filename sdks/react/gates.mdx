...
Renders children only when the customer is entitled to a feature. Uses `useCan()` internally with the provider's `entitlements:read` customer session.
...
<Info>
Customer-session entitlement checks use HTTP and omit cost, revenue, and margin fields. Refresh the query after billing mutations or on your application's own invalidation signal.
</Info>
