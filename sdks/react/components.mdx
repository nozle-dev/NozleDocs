...
<Info>
Checkout components use `BillingPortalProvider` with a publishable key. Product-credit read components use `BillingProvider` with a short-lived `customerSessionToken`; a bare publishable key cannot read product-credit balances or ledger operations. Customer sessions do not authenticate the legacy `BillingPortal` as a whole.
</Info>
