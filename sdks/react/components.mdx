...
<Info>
Checkout components use `BillingPortalProvider` with a publishable key. Product-credit read components use `BillingProvider` with a short-lived `customerSessionToken`; a bare publishable key cannot read balances or ledger operations.
</Info>
...
|------|------|---------|-------------|
| `customerId` | `string` | -- | Customer identifier |
| `creditSystemCode` | `string` | -- | Adds the product-credit balance section when provided |
...
|------|------|---------|-------------|
| `creditSystemCode` | `string` | **required** | Credit System code |
| `showUnavailable` | `boolean` | `true` | Include scheduled, expired, or exhausted source history |
...
<Info>
The older `CreditHistory` and `useCredits` exports read the legacy Lago wallet adapter. Use `CreditUsageHistory` and the `useCredit*` hooks for product Credit Systems.
</Info>
