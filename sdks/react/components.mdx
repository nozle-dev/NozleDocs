...
<Info>
`PricingTable` uses a publishable key for the public plan catalog. Every customer-facing billing component uses a short-lived `customerSessionToken`; publishable and secret keys are rejected as browser billing credentials.
</Info>
...
| `apiBaseUrl` | `string` | `'https://api.nozle.app'` | API base URL |
| `customerSessionToken` | `string` | `''` | Scoped customer session for preview and checkout |
| `apiKey` | `string` | `''` | Deprecated alias accepted only for a `csess_...` token |
...
| `onScheduled` | `() => void` | -- | Called when checkout delegates a Lago-classified downgrade to the scheduled change endpoint |
| `onConfirm` | `() => void` | -- | Backward-compatible callback after checkout creation, credit-funded completion, or scheduling |
...
Aggregated portal view that composes `CurrentPlan`, `UsageDashboard`, `InvoiceList`, and the existing Lago wallet `CreditBalance`. It requires the matching customer session through `BillingPortalProvider`.
