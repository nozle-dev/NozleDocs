...
Create a subscription for the authenticated customer session. Calls `POST /v1/subscribe` with the plan code and the provider customer ID.

<Warning>
This hook requires a customer session with `subscriptions:write`. A publishable key is never used for the request. Trusted backend code may instead call the endpoint with a secret key.
</Warning>
