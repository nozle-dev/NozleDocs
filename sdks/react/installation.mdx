...
```bash
npm install @nozle-js/react@0.8.0 react react-dom @stripe/stripe-js @stripe/react-stripe-js
```

Version `0.8.0` defaults catalog requests to `https://api.nozle.app/engine`.
...
| `baseUrl` | `string` | `https://api.nozle.app/engine` | Nozle API origin used for public catalog requests. |
