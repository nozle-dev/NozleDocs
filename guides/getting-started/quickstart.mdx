...
For public pricing, checkout, and the existing `BillingPortal`, a publishable key is used. For the new product-credit balances and operation history, mint a customer session in your backend and pass it alongside that publishable key to `BillingProvider`:
...
The product-credit components use HTTP only. The session does not authenticate the full legacy portal or Centrifugo.
</Step>
