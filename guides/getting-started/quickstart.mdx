...
| `csess_` | Customer session | Exact-customer browser operations |

<Warning>
Never expose your `sk_` secret key in client-side code. Use `pk_` only for public pricing and mint a short-lived `csess_` token for every customer-specific React operation.
</Warning>
</Step>
...
Use a publishable key for public pricing only. Mint a customer session in your backend and pass it alongside the publishable key for checkout, billing, entitlement, and product-credit operations:

```tsx title="app/providers.tsx"
import { BillingProvider, ProductCreditBalance } from '@nozle-js/react';

export function Providers({ children, sessionToken }: { children: React.ReactNode; sessionToken: string }) {
  return (
    <BillingProvider publishableKey="pk_..." customerSessionToken={sessionToken} customerId="customer_123">
...
Customer-session components use HTTP only. A bare publishable key cannot read or mutate any customer, and the session does not authenticate Centrifugo.
</Step>
