...
|------------|---------|--------|
| `pk_` | Publishable key | Browser plan catalog only |
| `sk_` | Secret key | Server-side only (Node.js, Python) |

<Warning>
Never expose your `sk_` secret key in client-side code. Use `pk_` only for public plan catalog components and mint a scoped `csess_` customer session for customer-facing billing components.
</Warning>
</Step>
...
Use a publishable key only for public pricing. Mint a scoped customer session in your backend for checkout, billing, entitlements, top-ups, and product-credit operations, then pass both credentials to `BillingProvider`:
...
Customer operations use HTTP only. The session does not authenticate Centrifugo and cannot access another customer.
</Step>
