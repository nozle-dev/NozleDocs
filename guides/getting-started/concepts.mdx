...
Entitlements are checked via the [`can()` endpoint](/guides/entitlements/can-endpoint) (REST) or the `useCan()` hook (React SDK). Browser checks use a customer session and ordinary authenticated HTTP.

<CardGroup cols={2}>
  <Card title="Entitlements Overview" href="/guides/entitlements/overview" />
  <Card title="Realtime security boundary" href="/guides/entitlements/real-time" />
</CardGroup>
...
Nozle uses three credential types to separate public, customer-bound, and server-only operations:

| Key prefix | Name | Use case |
|------------|------|----------|
| `pk_` | Publishable | Safe only for public client-side plan catalog reads. It cannot access customer data or write routes. |
...
| `csess_` | Customer session | Short-lived browser credential bound to one customer and explicit billing scopes. Mint it from your backend. |
