...
The browser must use a scoped Nozle customer session (`csess_...`) for checkout. The customer ID must match the token subject. A Nozle publishable key is valid only for the plan catalog. If you mount embedded Stripe checkout yourself, your app also needs the Stripe publishable key for the same Stripe account connected in Nozle.
