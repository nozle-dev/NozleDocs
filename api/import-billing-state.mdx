...
The referenced customer, plan, and optional Entity must already exist in the authenticated organization. The period must contain the current time. When a recurring credit grant uses the same cadence as the plan, the imported period is authoritative: the opening balance expires at `current_period_end`, and subsequent grant periods continue from that boundary. Independent grant cadences remain independent—for example, monthly credits on an annual plan still reset monthly—and lifetime grants remain non-expiring.
