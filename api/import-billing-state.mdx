...
The referenced customer, plan, and optional Entity must already exist in the authenticated organization. The period must contain the current time.
...
Set `dry_run` to `true` to validate the request without writing subscription state.
