...
**Auth:** secret key only. Publishable keys and customer sessions are rejected.
...
<Note>
  Also available to trusted backends as `POST /subscriptions/change` (alias). Do not expose either route to browser sessions.
</Note>
