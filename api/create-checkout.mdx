...
Create a payment-gated plan checkout. Browser callers must use a customer session bound to the exact customer.

**Auth:** customer session with `checkout:create`, or a secret key. Publishable keys are always rejected.

<Info>
With a customer session, `customer_id` must match the session subject. Nozle never returns your Stripe secret key to the browser. The response contains a hosted URL or Stripe Checkout `client_secret`, or reports an immediately completed or scheduled transition.
</Info>
