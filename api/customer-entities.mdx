...
`seat_sync` is `null` when no configured seat metric currently applies. `pending`, `processing`, `failed`, and `dead_lettered` describe durable event delivery; aggregate reconciliation separately reports `pending`, `in_sync`, `drift`, `repairing`, or `failed`.
